...
import { updateQty, removeItem, clearCart, itemSubtotal, cartTotal } from '@/lib/cart'
...
export default function CartPage() {
  ...
  function handleIncrement(productId: string) {
    ...
    if (!item) return
    setCart(updateQty(cart, productId, item.quantity + 1))
  }
  ...
  function handleRemove(productId: string) {
    setCart(removeItem(cart, productId))
  }
      ...
      {items.length === 0 ? (
        ...
        <>
          <ul className="space-y-4">
            {items.map((item) => (
              <li key={item.productId} className="bg-white border border-gray-100 rounded-lg p-5 flex items-center gap-5 shadow-sm">
                {/* Image */}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-gray-500 mt-1">${item.price.toFixed(2)} c/u</p>
                </div>
          ...
          <div className="pt-6 border-t border-gray-100">
            <div className="flex items-center justify-between mb-6">
              ...
              <span className="text-2xl font-bold text-gray-900">${cartTotal(cart).toFixed(2)}</span>
            </div>
